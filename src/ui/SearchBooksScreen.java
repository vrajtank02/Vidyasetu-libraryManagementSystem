...
public class SearchBooksScreen extends JDialog {

    private JTextField txtSearch;
    private JButton btnPerformSearch;
    private JTable table;
    private DefaultTableModel tableModel;
    private BookDAO bookDAO;

    public SearchBooksScreen(JFrame parentFrame) {
        super(parentFrame, "Search Books | Vidyasetu LMS", true);
        setSize(1000, 650);
        ...
        JLabel lblHeader = new JLabel("Search Library Catalog");
        ...
        JLabel lblSearch = new JLabel("Enter Keyword (Title, Author, ISBN):");
        ...
        btnPerformSearch = new JButton("Search");
        btnPerformSearch.setFont(new Font("SansSerif", Font.BOLD, 16));
        btnPerformSearch.setBackground(new Color(33, 150, 243));
        btnPerformSearch.setForeground(Color.WHITE);

        btnPerformSearch.addActionListener(e -> executeSearch());
        txtSearch.addActionListener(e -> executeSearch());

        searchBarPanel.add(lblSearch);
        searchBarPanel.add(txtSearch);
        searchBarPanel.add(btnPerformSearch);
        ...
        table.getAccessibleContext()
                .setAccessibleName("Search Results Table. Use arrow keys to select a row to Edit or Manage Copies.");
    ...
    private void executeSearch() {
        ...
        if (books.isEmpty()) {
            table.getAccessibleContext().setAccessibleName("No results found for " + keyword);
            JOptionPane.showMessageDialog(this, "No books found matching '" + keyword + "'.", "No Results",
                    JOptionPane.INFORMATION_MESSAGE);
            table.getAccessibleContext().setAccessibleName("Search Results Table.");
        } else {
            table.getAccessibleContext().setAccessibleName(
                    books.size() + " results found for " + keyword + ". Use arrow keys to navigate.");
            for (Book b : books) {
                Object[] rowData = {
                        b.getBookId(),
    ...
    private String formatISBN(String isbn) {
        if (isbn == null)
            return "";
        if (isbn.length() == 13) {
            return isbn.substring(0, 3) + "-" + isbn.substring(3, 4) + "-" + isbn.substring(4, 8) + "-"
                    + isbn.substring(8, 12) + "-" + isbn.substring(12);
        } else if (isbn.length() == 10) {
            return isbn.substring(0, 1) + "-" + isbn.substring(1, 5) + "-" + isbn.substring(5, 9) + "-"
                    + isbn.substring(9);
        }
        return isbn;
    }
    ...
    private void editSelectedBook() {
        ...
        String formattedIsbn = (String) tableModel.getValueAt(selectedRow, 1);
        String rawIsbn = formattedIsbn.replace("-", ""); // Strip dashes for the Book model
        ...
        Book bookToEdit = new Book(id, rawIsbn, title, author, publisher, pubDate);
